...
func (s Service) Review(ctx context.Context, action contracts.Action) contracts.Decision {
	if s.Assessor == nil {
		return s.record(ctx, action, contracts.Decision{Outcome: contracts.DecisionError, Reason: "approval assessor is unavailable"})
	}
	assessment, err := s.Assessor.Assess(ctx, action)
	if err != nil {
		return s.record(ctx, action, contracts.Decision{Outcome: contracts.DecisionError, Reason: fmt.Sprintf("approval did not complete: %v", err)})
	}
...
func (s Service) record(ctx context.Context, action contracts.Action, decision contracts.Decision) contracts.Decision {
	...
	if s.Store == nil {
		return contracts.Decision{Outcome: contracts.DecisionError, Reason: "approval audit store is unavailable", CreatedAt: decision.CreatedAt}
	}
	id, err := s.Store.RecordDecision(ctx, action, decision)
	if err != nil {
		return contracts.Decision{Outcome: contracts.DecisionError, Reason: fmt.Sprintf("approval audit write failed: %v", err), CreatedAt: decision.CreatedAt}
	}
